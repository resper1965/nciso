...
CREATE TABLE IF NOT EXISTS credentials_registry (
  ...
  CONSTRAINT check_user_or_team CHECK (
    (holder_type = 'user' AND holder_id IS NOT NULL) OR
    (holder_type = 'team' AND holder_id IS NOT NULL)
  )
);
...
-- Políticas básicas (permitir acesso baseado em tenant_id)
...
CREATE POLICY "evaluations_tenant_policy" ON evaluations
  FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::text);

CREATE POLICY "technical_documents_tenant_policy" ON technical_documents
  FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::text);

CREATE POLICY "teams_tenant_policy" ON teams
  FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::text);

CREATE POLICY "credentials_registry_tenant_policy" ON credentials_registry
  FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::text);

CREATE POLICY "privileged_access_tenant_policy" ON privileged_access
  FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::text);
